...
for i,child in enumerate(root):
    ...
    if('nuclide' in child.tag):
        ...
        if(Nuclides.get(nuclide).getDecayConstant() < 0.0001): continue
        activity = Quantity.of(10000,"Bq")
        ...
        plt.clf()
        plt.vlines(e_values_bnl, ymin=0, ymax=i_values_bnl, linewidth=3, label="BNL", alpha=0.7)
        plt.vlines(e_values_sandia, ymin=0, ymax=i_values_sandia, linewidth=3, linestyle='--', label="Sandia", alpha=0.7)
        plt.xlabel('Energy')
        plt.ylabel('Intensity')
        plt.yscale('log')
        plt.saveas('figures/'child.attrib['symbol']+'.png')